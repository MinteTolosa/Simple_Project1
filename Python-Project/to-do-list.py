...
def main(page: ft.Page): # Fixed capitalization
    page.title = "To-Do List"
    page.horizontal_alignment = ft.CrossAxisAlignment.CENTER # Fixed typo

    new_task = ft.TextField(hint_text="Task", expand=True)
    priority_dropdown = ft.Dropdown(
        width=120,
        value="Must", # Fixed case mismatch to match dictionary keys
        options=[ft.dropdown.Option(p) for p in priority_order],
    )
    ...
    def add_clicked(e):
        ...
        # Removed extra closing bracket below
        label = ft.Text(f"[{priority}] {new_task.value}", color=color[priority])

        delete_button = ft.IconButton(icon=ft.Icons.DELETE)
        task_row = ft.Row(controls=[label, delete_button], expand=True)
    ...
    # Moved page.add outside of add_clicked so layout loads on startup
    page.add(
        ...
        ]), # Fixed closing parentheses and semicolons
        tasks_view, 
    )

ft.app(target=main) # Fixed target runner function
